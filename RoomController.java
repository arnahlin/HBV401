...
public class RoomController {
	...
	public ArrayList<Room> searchByPrice(int priceBot, int priceTop) throws SQLException {
		...
		String sql = "SELECT r.roomID, r.hotelID, r.price, r.available, r.type , h.name from hotel h, room r where r.hotelID = h.hotelID AND available = \"y\" AND price BETWEEN "
				+ priceBot + " AND " + priceTop + ";";
	...
	public ArrayList<Room> searchByType(String type) throws SQLException {
		...
		Statement stmt = conn.createStatement();
		ArrayList<Room> temp = new ArrayList<Room>();
		String sql = "SELECT r.roomID, r.hotelID, r.price, r.available, r.type, h.name from hotel h, room r where r.hotelID = h.hotelID AND available = \"y\" AND type = \""
				+ type + "\";";
		ResultSet srs = stmt.executeQuery(sql);
		while (srs.next()) {
			Room room = new Room();
	...
	public static void main(String[] args) throws Exception {
		...
		//System.out.println(Arrays.toString(test.searchByPrice(15000,
		//25001).toArray()));
			...
			if (searchType.equals("v")) { // leit eftir verði
				...
				while (!validPrice) {
					...
					if (price) {
						...
						for (int i = 0; i < rooms.size(); i++) {
							...
							System.out.printf("%-10s", rooms.get(i).getType());
							System.out.printf("%-12s", rooms.get(i).getHotelName());
							System.out.println();
						}
						return;
					} else {
				...
				while (!validSize) {
					...
					if (size.equals("small") || size.equals("medium") || size.equals("large")) {
						...
						System.out.println("Sládu inn staerd: 'small', 'medium', 'large'");
					}
				}
			} else {
